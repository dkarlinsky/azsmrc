...
import lbms.azsmrc.remote.client.internat.*;
...
{    
	...
	private LocaleUtil localeUtil = LocaleUtil.getSingleton();
			...
			final 				AddTorrentContainer 			container ) 
	{
		...
		try {
			candidates = localeUtil.getTorrentCandidates(container.getTorrent());
		} catch (UnsupportedEncodingException e) {
			MessageBox mb = new MessageBox(display.getActiveShell(), SWT.OK | SWT.ICON_ERROR);
		...
		encodingShell = new Shell(display,SWT.RESIZE | SWT.DIALOG_TRIM | SWT.PRIMARY_MODAL);
		...
		final Table table = new Table(encodingShell, SWT.SINGLE | SWT.FULL_SELECTION | SWT.BORDER | SWT.V_SCROLL);
	 ...
	 * @param updater
	 */
	public static void open(final Display display, final AddTorrentContainer container){
		if(display == null) return;
		if (instance == null || instance.encodingShell == null || instance.encodingShell.isDisposed()){
