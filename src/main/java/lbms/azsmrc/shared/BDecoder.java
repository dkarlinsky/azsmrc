...
public class BDecoder {
	  ...
	  case 'd' :
		...
		try{
			...
			while ((tempByteArray = (byte[]) decodeInputStream(bais, nesting+1)) != null) {
			  ...
			  CharBuffer	cb = RemoteConstants.BYTE_CHARSET.decode(ByteBuffer.wrap(tempByteArray));
  ...
  private long getNumberFromStream(InputStream bais, char parseChar) throws IOException {
	...
	CharBuffer	cb = RemoteConstants.DEFAULT_CHARSET.decode(ByteBuffer.wrap(tempArray));
