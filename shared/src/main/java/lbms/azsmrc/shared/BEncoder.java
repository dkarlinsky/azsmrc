...
BEncoder
{
		...
		throws IOException
	{
		...
		if ( object instanceof String || object instanceof Float){
			...
			ByteBuffer	bb 	= RemoteConstants.DEFAULT_CHARSET.encode( tempString );

			write(baos,RemoteConstants.DEFAULT_CHARSET.encode(String.valueOf(bb.limit())));
					...
					if ( o_key instanceof byte[]){
						...
						if ( byte_keys ){
		   					...
		   					try{
									...
									encode( baos, RemoteConstants.BYTE_CHARSET.encode(key));
			...
			//write out the l
			   baos.write('i');
			   write(baos,RemoteConstants.DEFAULT_CHARSET.encode(tempLong.toString()));
			   baos.write('e');
			...
			//write out the l
			baos.write('i');
			write(baos,RemoteConstants.DEFAULT_CHARSET.encode(tempInteger.toString()));
			baos.write('e');

	   }else if(object instanceof byte[]){

			byte[] tempByteArray = (byte[])object;
			write(baos,RemoteConstants.DEFAULT_CHARSET.encode(String.valueOf(tempByteArray.length)));
			baos.write(':');
		...
		Object		o2 )
	{
		...
		if ( o1 instanceof Long ){
			...
			System.out.println( "Invalid type: " + o1 );
